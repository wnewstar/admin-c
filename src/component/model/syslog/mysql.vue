...
export default {
    ...
    created () {
        ...
        this.api = this.$apiback.syslog.mysql
        this.bit = 0 +
...
<template>
    <base-model
        ...
        <template slot="datahead" slot-scope="base">
            <th>编号</th>
            <th>耗时</th>
            <th>HOST</th><th>PORT</th><th style="width: 50%;">SQL</th><th>BIND</th><th>创建时间</th>
        </template>
        <template slot="databody" slot-scope="base">
            <td class="center">{{ base.item.id }}</td>
            <td>{{ base.item.ms }}</td>
            <td>{{ base.item.host }}</td>
            <td>{{ base.item.port }}</td><td>{{ base.item.text }}</td><td>{{ base.item.bind }}</td><td>{{ base.item.time_create | datetime }}</td>
        </template>
    </base-model>
</template>
